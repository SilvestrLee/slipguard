...
**Still open, not yet authorized (as of 2026-07-26 — see correction below):** Capability B (generative planning from a live fixture/odds/market universe) remains unauthorized and out of scope — no data source exists, and none of the above resolves that. Product/compliance policy on planner stopping, suitability, and abandonment messaging (`docs/02-architecture/U-07.2-PLANNER-PRODUCT-DECISION-REGISTER.md`'s DR-01, Compliance assessment delivered, not yet decided) remains undecided.

**Correction (`PO-U23-001A`, 2026-08-07):** the paragraph above describes the 2026-07-26 starting position only and is now stale as a description of current state — retained unedited for historical accuracy of that original decision, per this repository's own established correction convention (`TASKS.md`'s identical note against this same paragraph). Capability B was re-authorized for architecture discovery at `PO-U08.1-001-R1` (2026-07-29), built out as **Programme U-17** ("Build an Accumulator" — `docs/02-architecture/U-17.1-DETERMINISTIC-MARKET-INTELLIGENCE-PLATFORM-ARCHITECTURE.md` through `U-17.5`, `TASKS.md`'s own `U-17`/`U-17.8` entries), and — per `PO-MVP-004` (2026-08-03) — is now **included in MVP v1.0**, gated behind `MARKET_WIDE_PLANNER_ENABLED` pending Product Office's own separate public-launch authorization (still `false` by default). A natural-language conversational entry layer for Capability B was subsequently delivered under `PO-U23-001` (2026-08-07): `App\Domain\AccumulatorConversation\*`, a deterministic (never AI-model-authoritative) interpreter producing the same `PlanningBrief` the existing Builder already consumes — **no live AI provider is integrated anywhere in this codebase**, a deliberate Product Office decision (`PO-U23-001` Decision 1), not an oversight; the interpreter/provider is a swappable interface specifically so that remains a future, separately-authorized decision. See `TASKS.md`'s `PO-U23-001` entry for the full implementation record.
