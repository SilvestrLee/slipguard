...
## Theme Exceptions (added `PO-U20.1-002`, 2026-08-06)

`PO-U20.1-001`'s Product Principle states every route inherits its appearance from the single global theme "unless Product Office explicitly authorizes an exception." Two exceptions exist today. Both are ratified here explicitly, per that requirement, rather than left as an implicit accident of implementation.

**Exception A — deliberately theme-constant surfaces.** `--surface-inverse` and `--gradient-cta` (above, and their prior entries in this document) hold the same value in both light and dark mode by design — the homepage Intelligence Section and every public CTA section are meant to read as a fixed, always-dark "spotlight" band regardless of site theme, an existing precedent this document already documented before this exception was formally named. **This is the only always-constant pair in the product.** The Dashboard's "Accumulator workspace" preview card (`dashboard.blade.php`, `bg-[image:var(--gradient-dashboard-feature)]`) was checked directly against this exception during `PO-U20.1-002`'s audit and found **not** to belong to it — `--gradient-dashboard-feature` has genuinely distinct light (`#6366f1 → #3730a3`) and dark (`#4338ca → #27245f`) values (`app.css`, both blocks) and already participates correctly in the global theme. It is a dark-toned card by visual design, not a theme exception, and required no correction.

**Exception B — the internal Filament Operations panel.** `app/Providers/Filament/OperationsPanelProvider.php` uses Filament's own native dark-mode mechanism, entirely independent of `window.SlipGuardTheme`. This is a genuine second theme system, in literal tension with this document's own line 14 ("never hardcode a colour... in a Blade view, Livewire component, or **Filament resource**"). It is ratified as an exception because the panel is internal-only staff tooling — not customer-facing, and therefore outside the customer-trust/premium-perception concern `PO-U20.1-001` exists to protect — consistent with `CLAUDE.md`'s existing "Internal operations use Filament" Locked Decision. Line 14's "Filament resource" wording refers to customer-visible Filament content only (e.g. `LabsFeatureResource` form fields rendered inside the panel's own chrome), not the panel's own chrome/theme, which this exception now explicitly excludes from this document's token-compliance requirement.
