...
- **Must complete before RC1** — 20 items, **10 now closed** (`PO-RC1-002`, this pass — all 7 Engineering items + 3 of 5 Compliance items). The convergence point where `PO-MVP-005`'s "NO GO today, GO WITH CONDITIONS realistically close" becomes an actual GO. **10 remain**: all 7 Operations items (`OP-01`–`OP-07`, no infrastructure exists yet), 2 Compliance items (`CO-01`/`CO-02`, real Terms/Privacy content — require legal authorship, cannot be closed by Engineering), and 1 Product Office item (`PO-01`).
...
| ID | Item | Owner | Priority | Est. effort | Dependencies | Acceptance criteria | Evidence required for closure |
|---|---|---|---|---|---|---|---|
| CO-01 | Real Terms of Service content, jurisdiction-neutral wording per this directive | Compliance/Legal `⚠` | Critical | Large | Real legal review — this plan does not itself author binding legal text, per `CO-MVP-001`'s own constraint | Real content live at `/terms`; includes the disclaimer clause (`CO-MVP-001` C-02), IP clause (supports `EN-05`), and age statement (supports `EN-06`) | Published document; content sign-off |
| CO-02 | Real Privacy Policy content, jurisdiction-neutral wording, including a cookies subsection per `CO-MVP-001`'s recommendation not to create a separate Cookie Policy | Compliance/Legal `⚠` | Critical | Large | Real legal review | Real content live at `/privacy` | Published document; content sign-off |
